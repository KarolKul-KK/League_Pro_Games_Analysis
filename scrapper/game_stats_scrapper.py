...
def get_left_team_stats(
    data: bs4.element.Tag, match_id: int, match_count_i: int
) -> pd.Series:
    """match_count_i is iteration in for loop when scrapper working."""

    l_team_name = l_team_result = (
        data.find("div", attrs={"class": "row rowbreak pb-3"})
        .text.strip()
        .replace("\n", "")
        .split("-")[0]
    )
    ...
    kills_l_counts = (
        data.find("div", attrs={"class": "col-2"})
        .text.replace(" ", "")
        .replace("\n", "")
    )
    ...
    towers_l_count = (
        data.find_all("div", attrs={"class": "col-2"})[1]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    ...
    dragons_l_count = (
        data.find_all("div", attrs={"class": "col-2"})[2]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    baron_l_count = (
        data.find_all("div", attrs={"class": "col-2"})[3]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    gold_l_count = (
        data.find_all("div", attrs={"class": "col-2"})[4]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    ...
    for i in range(5):
        try:
            bans_l_team.append(
                data.find_all("div", attrs={"class": "col-10"})[0]
                .find_all("a")[i]
                .get("title")
                .split()[0]
            )
    ...
    for i in range(5):
        picks_l_team.append(
            data.find_all("div", attrs={"class": "col-10"})[1]
            .find_all("a")[i]
            .get("title")
            .split()[0]
        )
...
def get_right_team_stats(
    data: bs4.element.Tag, match_id: int, match_count_i: int
) -> pd.Series:

    r_team_name = (
        data.find("div", attrs={"class": "col-12 red-line-header"})
        .text.strip()
        .replace("\n", "")
        .split("-")[0]
    )
    ...
    kills_r_count = (
        data.find_all("div", attrs={"class": "col-2"})[8]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    towers_r_count = (
        data.find_all("div", attrs={"class": "col-2"})[9]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    ...
    dragons_r_count = (
        data.find_all("div", attrs={"class": "col-2"})[10]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    barons_r_count = (
        data.find_all("div", attrs={"class": "col-2"})[11]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    gold_r_count = (
        data.find_all("div", attrs={"class": "col-2"})[12]
        .text.replace(" ", "")
        .replace("\n", "")
    )
    ...
    for i in range(5):
        try:
            bans_r_team.append(
                data.find_all("div", attrs={"class": "col-10"})[2]
                .find_all("a")[i]
                .get("title")
                .split()[0]
            )
    ...
    for i in range(5):
        picks_r_team.append(
            data.find_all("div", attrs={"class": "col-10"})[3]
            .find_all("a")[i]
            .get("title")
            .split()[0]
        )
...
def get_players_stats(
    ...
    match_id: int,
    left_team_picks: list,
    right_team_picks: list,
    match_count_i: int,
) -> pd.DataFrame:
    ...
    gold_distribution = data.find_all("table", attrs={"class": "small_table"})[
        0
    ].find_all("td")
    ...
    damage_distribution = data.find_all("table", attrs={"class": "small_table"})[
        1
    ].find_all("td")
