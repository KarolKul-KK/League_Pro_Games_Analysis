...
from game_stats_scrapper import (
    get_match_count,
    get_general_data,
    get_left_team_stats,
    get_right_team_stats,
    get_players_stats,
    make_dir,
)
...
def main() -> None:
    ...
    for url in urls:
        ...
        for i in range(match_count):
            ...
            time.sleep(1)
            left_team_stats = get_left_team_stats(
                data, general_data_series["Match_id"], i
            )
            time.sleep(1)
            right_team_stats = get_right_team_stats(
                data, general_data_series["Match_id"], i
            )
            time.sleep(1)
            player_stats = get_players_stats(
                data,
                general_data_series["Match_id"],
                left_team_stats["Picks"],
                right_team_stats["Picks"],
                i,
            )
            time.sleep(1)
            print(general_data_series["Date"])

            general_table = pd.concat(
                [general_table, pd.DataFrame(general_data_series).T], ignore_index=True
            )
            left_team_stats_table = pd.concat(
                [left_team_stats_table, pd.DataFrame(left_team_stats).T],
                ignore_index=True,
            )
            right_team_stats_table = pd.concat(
                [right_team_stats_table, pd.DataFrame(right_team_stats).T],
                ignore_index=True,
            )
            players_stats_table = pd.concat(
                [players_stats_table, player_stats], ignore_index=True
            )

            driver.find_element_by_xpath(
                f'//*[@id="gameMenuToggler"]/ul/li[{i+3}]/a'
            ).click()
